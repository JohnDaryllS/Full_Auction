    ...
    <main class="container auction-container <?php echo $is_admin_view ? 'admin-view' : ''; ?>">
        ...
        <?php if ($type_filter > 0): ?>
            <div class="type-breadcrumb">
                <a href="auction.php">All Auctions</a> &raquo; <span><?= htmlspecialchars($type_name) ?></span>
            </div>
            
            <div class="type-header">
                <div class="type-image-header">
                    <img src="images/type-images/<?= htmlspecialchars($type_data['image']) ?>" alt="<?= htmlspecialchars($type_name) ?>">
                </div>
                <div class="type-info-header">
                    <h1><?= htmlspecialchars($type_name) ?></h1>
                    <p><?= htmlspecialchars($type_data['description']) ?></p>
                </div>
            </div>
        <?php endif; ?>
...
function displayAuctionItem($item, $isAdminView) {
    ...
    if ($item['is_limited']) {
        echo '<span class="limited-badge">Limited Edition</span>';
    }
    ...
    if ($item['type_name']) {
        echo '<div class="auction-type-badge">' . htmlspecialchars($item['type_name']) . '</div>';
    }
